...
privatearea_urls = [
    url(r'^dashboard/$', login_required(MyDashboardView.as_view()), name="pushetta-dashboard"),
    ...
    url(r'^channels/(?P<channel_name>[\w|\W]+)/push/$', login_required(MyChannelsView.as_view()),
        name="my-channel-pushmessage"),
    url(r'^channels/(?P<channel_name>[\w|\W]+)/$', login_required(MyChannelsView.as_view()), name="my-channelview"),
    url(r'^requests/approve/$', login_required(ApproveRequestsView.as_view()), name="my-approve-requests"),
]

# Url per la gestione della sottoscrizione ai Canali con gestione
# del device di provenienza (Android, iOs, Wp, ...)
channel_subscription_urls = [
    url(r'^dispatch/(?P<channel_name>[\w|\W]+)/$', ChannelSubscriberDispatcher.as_view(), name="channel-subscription-dispatcher"),
    url(r'^android/(?P<channel_name>[\w|\W]+)/$', AndroidSubscribe.as_view(), name="android-subscribe"),
    url(r'^ios/(?P<channel_name>[\w|\W]+)/$', iOSSubscribe.as_view(), name="ios-subscribe"),
    url(r'^safari/(?P<channel_name>[\w|\W]+)/$',login_required(SafariSubscribe.as_view()), name="safari-subscribe"),
    url(r'^chrome/(?P<channel_name>[\w|\W]+)/$', login_required(ChromeSubscribe.as_view()),name="chrome-subscribe"),
]
...
safari_urls = [
    url(r'^/devices/(?P<deviceToken>[\w|\W]+)/registrations/web.com.pushetta$', csrf_exempt(SafariPushService.as_view()), name='safari-token-register'),
    url(r'^/pushPackages/web.com.pushetta$', csrf_exempt(SafariPackageDownloadService.as_view()), name='safari-package-download'),
    url(r'^/log', csrf_exempt(SafariLogService.as_view()), name='safari-log'),
]

browser_urls = [
    # Endpoint per la registrazione anonima del solo device
    url(r'^register/device$', csrf_exempt(WebPushRegistration.as_view()), name='browser-registration-device'),
    url(r'^register/(?P<device_id>[\w|\W]+)/(?P<channel_name>[\w|\W]+)/$', login_required(csrf_exempt(WebPushRegistration.as_view())), name='browser-get-registration'),
    url(r'^register$', login_required(csrf_exempt(WebPushRegistration.as_view())), name='browser-registration'),
]
...
urlpatterns = [
    ...
    url(r'^pushetta-search/$', ChannelSearchView.as_view(), name="pushetta-search"),
    url(r'^pushetta-api/', TemplateView.as_view(template_name="api-docs.html"), name="pushetta-api-doc"),
    url(r'^pushetta-docs/$', TemplateView.as_view(template_name="docs.html"), name="pushetta-docs"),
    url(r'^pushetta-downloads/$', TemplateView.as_view(template_name="downloads.html"), name="pushetta-downloads"),
